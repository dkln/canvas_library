...
Tested on iPhone. Demo's do not run very smooth but is is a beginning!
...
== Todo/notes
* add bitmap caching support to shapes (like cacheAsBitmap in Flash) to increase performance (http://dev.w3.org/html5/2dcontext/#pixel-manipulation)
* refactor canvaslib.Shape so all drawing-command's are collected/cached for more performance
* add more easing modules
* finalize text_field class
* add more transformation options to shape and bitmap
* shadows
* lineWidth / lineCap etc.
* focus management
* globalCompositeOperation etc.
